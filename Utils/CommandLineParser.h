...
namespace clp
{
	...
	{
	public:
		IllegalArgumentException(const std::string& message = "") : Exception("IllegalArgumentException: " + message) {}
	};

	class ArgumentNotFoundException: public Exception
	{
	public:
		ArgumentNotFoundException(const std::string& message = "") : Exception("ArgumentNotFoundException: " + message) {}
	};

	class CommandLineArgumentParser
	{
	public:
		CommandLineArgumentParser(int argc, char** argv) : args(argv, argv + argc) {}
		...
		bool GetFlag(const std::string& option) const { return std::find(args.begin(), args.end(), option) != args.end(); }
